#!/bin/bash
# XIPSERCLOUD - SKRIP INTI V4.0 (PRODUCTION)
# Dibuat untuk stabilitas, Ngrok terjamin, dan manajemen server otomatis di Termux.
...
# Fungsi untuk menginstal paket dengan pengecekan dan retry
function install_package() {
    local pkg_name=$1
    if command -v $pkg_name &> /dev/null; then
        echo -e "${GREEN}[SETUP]${NC} Paket $pkg_name sudah terinstal."
    else
        echo -e "${YELLOW}[SETUP]${NC} Menginstal paket $pkg_name...${NC}"
        pkg install $pkg_name -y
        if command -v $pkg_name &> /dev/null; then
            echo -e "${BG_GREEN}✅ Sukses:${NC} $pkg_name berhasil diinstal."
        else
            echo -e "${BG_RED}❌ Gagal:${NC} $pkg_name gagal diinstal. Periksa koneksi!."
            exit 1
        fi
    fi
}
...
# --- SHUTDOWN AMAN (Fitur Vital untuk Pencegahan Dunia Korup) ---
function safe_shutdown() {
    ...
    if screen -list | grep -q ".$SCREEN_SESSION_MC"; then
        ...
        # Tunggu server benar-benar mati (maks 20 detik)
        for i in {1..20}; do
            if ! screen -list | grep -q ".$SCREEN_SESSION_MC"; then
    ...
    # Matikan semua sesi screen lain yang terkait
...
# --- MONITORING SYSTEM (Fitur Nyata Termux) ---
function monitor_server() {
    MAX_RUNTIME_SECONDS=$((12 * 60 * 60)) # Batas waktu 12 jam
    LOW_BATTERY_THRESHOLD=5              # 5%
    ...
    while true; do
        # Pengecekan sesi Minecraft (Apakah Server Crash/Mati?)
        if ! screen -list | grep -q ".$SCREEN_SESSION_MC"; then
        ...
        # Pengecekan Baterai (Membutuhkan termux-api)
...
# --- MANAJEMEN ADMIN (Membaca file admin_database.txt) ---
function process_admins() {
    echo -e "${CYAN}MEMPROSES DATABASE ADMIN...${NC}"
    
    if [ -f "admin_database.txt" ]; then
        # Beri waktu sebentar agar konsol server siap
        sleep 5 
...
function main_setup_and_start() {
    ...
    # Aikar's Flags untuk optimasi PaperMC
    ...
    echo -e "${BLUE}==========================================================${NC}"
    echo -e "${BOLD}${CYAN}          [XIPSERCLOUD] Setup & Launch Script V4.0        ${NC}"
    echo -e "${BLUE}==========================================================${NC}"
    ...
    # --- TAHAP 1: INSTALASI DEPENDENSI (Self-Healing) ---
    echo -e "${GREEN}[SETUP]${NC} Memastikan paket esensial terinstal...${NC}"
    install_package openjdk-17
    install_package wget
    install_package screen
    install_package jq
    install_package curl
    install_package termux-api # Untuk monitoring baterai
    ...
    # Modifikasi server.properties (Wajib untuk server non-PC/non-root)
    if [ ! -f "server.properties" ]; then
         echo -e "${GREEN}[SETUP]${NC} Server.properties tidak ada. Menjalankan sebentar untuk generate..."
    ...
    sed -i 's/online-mode=true/online-mode=false/' server.properties # Wajib untuk Geyser/Floodgate
    sed -i 's/^server-ip=.*$/server-ip=/' server.properties 

    # --- TAHAP 3: INSTALASI PLUGIN (Geyser & Floodgate untuk Bedrock) ---
    echo -e "${YELLOW}--- INSTALASI PLUGIN MULTI-PLATFORM (Wajib untuk HP & PC) ---${NC}"
    mkdir -p plugins
    ...
    # 1. Memulai Ngrok (Log diarahkan ke ngrok.log)
    rm -f $NGROK_LOG_FILE
    ...
    # 2. Memulai Monitor
    export -f safe_shutdown monitor_server get_ram get_external_ip process_admins set_ram install_package
    ...
    if [ -z "$NGROK_URL" ]; then
        ...
        echo -e "${RED}  Server berjalan di latar belakang, tapi tunneling gagal. Cek ulang TOKEN Ngrok Anda!${NC}"
    else
